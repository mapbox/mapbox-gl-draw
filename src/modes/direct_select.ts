...
import type { SelectedDrawModeFeature } from '../types/types';
...
interface State {
  ...
  dragMoving: boolean;
  feature: SelectedDrawModeFeature;
...
const DirectSelect = {
  ...
  fireActionable: function (state: State) {
  ...
  startDragging: function (state: State, e) {
  ...
  stopDragging: function (state: State) {
  ...
  onVertex: function (state: State, e) {
  ...
  onMidpoint: function (state: State, e) {
    this.startDragging(state, e);
    const about = e.featureTarget.properties;
  ...
  onFeature: function (state: State, e) {
    if (state.selectedCoordPaths.length === 0) this.startDragging(state, e);
    else this.stopDragging(state);
  },

  dragFeature: function (state: State, e, delta) {
    moveFeatures(this.getSelected(), delta);
    state.dragMoveLocation = e.lngLat;
  },

  dragVertex: function (state: State, e, delta) {
  ...
  clickActiveFeature: function (state: State) {
    state.selectedCoordPaths = [];
  ...
  toDisplayFeatures: function (state: State, geojson, push) {
